...
public class HuobiStatService {
    ...
    //输出表格
    public List outputWorkbook(Workbook workbook){
    ...
    public List<ArrayList<Object>> generateWorksheetsSum(ArrayList<String> user){
        ...
        for (ArrayList i:dataList2) {
            ...
            result.add("");
            result.add(i.get(1));
            results.add(dataList1.size(), result);
        }
            ...
            result.add("");
            result.add("-");
            results.add(dataList1.size() + dataList2.size(), result);
        }
...
//        //生成Excel表格
//        String[] headerString = new String[] {"时间", "操作明细", "操作类型", "起始币种", "起始数量", "结束币种", "结束数量", "币种余额", "来源地址/目标地址"};
//        ArrayList<String> header = new ArrayList<>();
//        for (int i = 0; i < headerString.length; i++) {
//            header.add(i,headerString[i]);
//        }
//        Sheet sheet = newBook.createSheet(user.get(1) + "_" + user.get(0));
...
//                int number = 0;
//                for (String i:header) {
//                    row.createCell(number).setCellValue(i);
//                    number++;
//                }
//            }else {
//                for (int i = 0; i < headerString.length; i++) {
...
//                }
//                for (Cell cell:row) {
//                    cell.setCellValue(results.get(cell.getRowIndex() - 1).get(cell.getColumnIndex()).toString());
//                }
//            }
//        }
        //添加表头
    ...
    public List<ArrayList<Object>> generateWorksheetsDetails(ArrayList<String> user){
        ...
        return results;
//        int rowSize = results1.size() + results2.size() + results3.size() + results4.size() + 8;
//        //制作表头
...
//            sheet.createRow(i);
//        }
//        int oneTable = results1.size() + 1;
//        int twoTable = results1.size() + results2.size() + 2;
//        int threeTable = results1.size() + results2.size() + results3.size() + 3;
//        //数据插入表格
//        for (Row row:sheet) {
//            if(row.getRowNum() == 0){
//                row.createCell(0).setCellValue("法币交易统计数据");
...
//                    number++;
//                }
//            } else if(row.getRowNum() == oneTable + 1){
//                row.createCell(0).setCellValue("币币交易统计数据");
//            } else if(row.getRowNum() == oneTable + 2){
//                int number = 0;
//                for (String i:header2) {
//                    row.createCell(number).setCellValue(i);
//                    number++;
//                }
//            } else if(row.getRowNum() == twoTable + 2){
//                row.createCell(0).setCellValue("充币记录统计数据");
//            } else if(row.getRowNum() == twoTable + 3) {
//                int number = 0;
//                for (String i : header3) {
//                    row.createCell(number).setCellValue(i);
//                    number++;
//                }
//            } else if(row.getRowNum() == threeTable + 3){
//                row.createCell(0).setCellValue("提币记录统计数据");
//            } else if(row.getRowNum() == threeTable + 4){
//                int number = 0;
//                for (String i:header4) {
//                    row.createCell(number).setCellValue(i);
//                    number++;
//                }
//            } else if(row.getRowNum() < oneTable + 1){
...
//                }
//                for (Cell cell:row) {
//                    cell.setCellValue(results1.get(cell.getRowIndex() - 2).get(cell.getColumnIndex()).toString());
//                }
//            } else if(row.getRowNum() < twoTable + 2 && row.getRowNum() > oneTable + 2){
//                for (int i = 0; i < header2.size(); i++) {
//                    row.createCell(i);
//                }
//                for (Cell cell:row) {
//                    cell.setCellValue(results2.get(cell.getRowIndex() - oneTable - 3).get(cell.getColumnIndex()).toString());
//                }
//            } else if(row.getRowNum() > twoTable + 3 && row.getRowNum() < threeTable + 3){
//                for (int i = 0; i < header3.size(); i++) {
//                    row.createCell(i);
//                }
//                for (Cell cell:row) {
//                    cell.setCellValue(results3.get(cell.getRowIndex() - twoTable - 4).get(cell.getColumnIndex()).toString());
//                }
//            } else if(row.getRowNum() < rowSize && row.getRowNum() > threeTable + 4){
//                for (int i = 0; i < header4.size(); i++) {
//                    row.createCell(i);
//                }
//                for (Cell cell:row) {
//                    cell.setCellValue(results4.get(cell.getRowIndex() - threeTable - 5).get(cell.getColumnIndex()).toString());
//                }
//            }
//        }
    }
    ...
    public List<ArrayList<Object>> generateC2CCurrency(ArrayList<String> user){
        ...
        for (String i:keyIn) {
            ...
            for (ArrayList<Object> j:dataList3) {
                if(j.get(6).toString().equals(i) && j.get(5).equals("买")){
        ...
        for (String i:keyOut) {
            ArrayList<String> timeData = new ArrayList<>();
            for (ArrayList<Object> j:dataList3) {
                if(j.get(6).toString().equals(i) && j.get(5).equals("卖")){
                    timeData.add(j.get(7).toString());
                }
            }
            Collections.sort(timeData);
    ...
    public List<ArrayList<Object>> generatrRecharge(ArrayList<String> user){
        ...
        for (String i:key) {
            ...
            for (ArrayList<Object> j:dataList2){
                if(j.get(4).equals(i) && j.get(3).equals("充币")){
    ...
    public List<ArrayList<Object>> generateWithdraw(ArrayList<String> user){
        ...
        for (ArrayList i:keyList){
            ...
            for (ArrayList j:dataList2) {
                if(i.get(0).equals(j.get(4)) && i.get(1).equals(j.get(1))){
                    timeData.add(j.get(6).toString());
                }
            }
            Collections.sort(timeData);
